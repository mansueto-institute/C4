...
if [[ "$METHOD" == "SPLIT" ]]; then 
  ...
  ./run.py -s ${s} -m path_frac -t 0.01 -n2500 -c $cycles --conv_iter 100 -x${x} --destrand_min 20 --destrand_max 50 --shading $shading -v 1 --tabu_length 3
else
  ...
  ./run.py -s ${s} -m polsby       -t 0.01 -x${x} -n$max_iter -c $cycles --conv_iter 1000 --destrand_min 5 --destrand_max 50 --tabu_length 10 --allow_trades  --shading $shading    
